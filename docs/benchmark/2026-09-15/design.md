...
순서대로(세로 스크롤 하나, 내부 스크롤 없음, 탭 줄바꿈 없음):
1. 헤더: 등급 배지 + 이름 + 배지들 + 특성 칩 전체 + 고정 4수치 줄(선택 구간 기준; 구간 칩을 여기서도 바꿀 수 있음) + 비교 한 줄(`중국 골드~에메랄드 3.61 · 글로벌 플래+ 4.22 · KR 플래+ 4.20`).
2. 큰 버튼 두 개: `덱 코드 복사`(기존, 대표 변형 코드) / `게임 위에 띄우기`(기존). 부분 코드 안내(기존).
3. 배치: 탭 `초반 Lv5 · 2-3` / `중반 Lv8 · 4-3` / `최종 Lv8`(편집 덱이 있을 때; 없으면 `최종`만). `9렙` 탭은 없앤다. 보드는 HexBoard(기존) + 히트맵 토글(`실측 배치` 스위치: positions가 있으면 각 유닛의 최빈 칸을 진하게, use%를 작게 표시; 편집 좌표와 다르면 점 표시). pet 칸은 소환물 스타일(회색 테두리, 별 없음). 탭 전환 시 코스트색 placeholder로 칸이 비지 않게 한다(§10).
4. 핵심 유닛 칩(keyUnits): `장로 드래곤 2성 67% · 아이템 2.9개`, 3성 비율이 30% 이상이면 `3성 NN%`를 앞에.
5. 레벨 도달(global.finalLevels 또는 levelDist): `8렙 47% · 9렙 47% · 10렙 5%` 한 줄. global 값에는 "기간·티어 미표기 집계" 도움말 아이콘.
6. 아이템 배분: 캐리 3명(carryRank) 크게, 대체 아이템(itemsBackup) 작게, 나머지 유닛은 접힘.
7. 조합 재료 우선순위(editorial.itemOrder) + 재료 칩 옆 추천 착용자 초상 1~2(itemWearers에서 부품→완성템→착용자 역추적은 하지 않고, `itemWearers` 항목이 그 재료를 포함하는 완성템일 때만).
8. 증강: `작가 추천`(editorial.augments, 아이콘+이름) / `통계 상위`(augmentStats: 아이콘 이름 `3.16등 · n=634`, 표본 300 미만 흐림; 단계별 평균은 접힌 상태에서 `2-1 / 3-2 / 4-2` 탭, stageLowSample은 흐림). 상단에 `중국 골드~에메랄드 9/14 기준 · 덱별 상위 5개만` 고지 한 줄.
9. 변형 덱(variants): 대표 외 변형 목록(유닛 얼굴 줄 + 4수치 + 편집 배지) — 탭하면 그 변형 보드를 3번 보드에 미리보기(좌표 없으면 히트맵 최빈 칸 배치).
10. 상대 덱(global.counters, 있을 때): `불리한 상대 3개`(우리 덱 이름으로 표시, 매칭 안 된 클러스터는 metatft name_string 그대로).
11. 글로벌 비교 카드: global.stats 세 스코프 8칸 분포 막대 + n. 없으면 `metatft에 대응 덱 없음(최고 유사도 NN%)`(기존 문구).
12. 작성자 원문(중국어, 접힘, 기존).
...
| 펼침 — 덱 목록 | 헤더에 티어 구간 이름(앱에서 마지막 선택한 구간, 탭하면 순환), 목록은 그 구간 등급순. 행: 등급 · 얼굴 줄 · (넓게: 이름 · 평균 등수 · 중국 한정 점). 고정한 덱이 맨 위. |
| 펼침 — 덱 요약 | 이름 · 등급 · 4수치 한 줄(평균/픽률/승률/TOP4) · 특성 칩 · 유닛(얼굴/이름/아이템, 넓게/좁게 기존) · 재료 5개 · `덱 코드 복사`. 초·중반 보드가 있으면 `초반/중반/최종` 세 칩으로 얼굴 줄만 전환(보드 그림은 그리지 않는다). |
| 티어 카드 | 소환사명 · 티어/LP · 최근 6판 칩(왼쪽 최신) + 칩 아래 ±LP · 평균 등수 큰 숫자 · `N판 · 톱4 NN%` · 새로고침(기존). 글자 최소 10sp(현재 8~9.5sp를 올린다). |
...
- **새 섹션 "빌드업"** 을 2번(버튼) 바로 아래에 둔다.
  1. 타이밍 한 줄: `5렙 2-5 · 6렙 3-2 · 7렙 3-5 · 8렙 4-2 · 9렙 4-6` + 강조 칩 `주 리롤 9렙 (판당 32회)`. 출처 글자 `글로벌 플래+ 3일`.
...
- 덱 요약의 `초반/중반/최종` 칩을 **레벨 칩**(buildup 이 있는 레벨)으로 바꾼다. 칩을 누르면 얼굴 줄이 그 레벨의 1순위 구성(글로벌 → 중국 → 작가 순으로 있는 것)으로 바뀌고, 그 아래 작은 글자로 `8렙 4-2 · 주 리롤 9렙`.
